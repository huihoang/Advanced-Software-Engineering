...
import { Input, Card, Typography, Empty, Row, Col, Button } from "antd";
...
const DoctorsPage = () => {
  ...
  const departments = Array.from(new Set(mockDoctors.map((d) => d.department)));
        ...
        <>
          {departments.map((dept) => (
            <DepartmentSlider
              key={dept}
              dept={dept}
              doctors={mockDoctors.filter((d) => d.department === dept)}
              onNavigate={(id: number) => navigate(`/doctors/${id}`)}
            />
          ))}
        </>
      ) : (
